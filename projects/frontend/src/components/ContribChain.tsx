...
export const ContribChain = ({ openModal, closeModal }: any) => {
  ...
  const deployContract = async () => {
    try {
      ...
      setDeploying(true)
      const factory = new ContribChainFactory({ defaultSender: activeAddress, algorand })
      const result = await factory.send.create.bare()
      const newId = Number(result.appClient.appId)
      setAppId(newId)
      enqueueSnackbar(`ContribChain deployed. App ID: ${newId}`, { variant: 'success' })
    } catch (e) {
  ...
  return (
    <dialog id="ContribChain_modal" className={`modal ${openModal ? 'modal-open' : ''} bg-slate-200`}>
      <form method="dialog" className="modal-box max-w-3xl">
        ...
        <div className="mt-2 flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            ...
            <input className="input input-bordered" type="number" value={appId} onChange={(e) => setAppId(e.target.value === '' ? '' : Number(e.target.value))} placeholder="Enter deployed ContribChain App ID" />
            {appAddress && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex flex-col gap-2 p-4 rounded-lg bg-white">
              ...
              <button className={`btn btn-accent ${deploying ? 'loading' : ''}`} disabled={deploying || !activeAddress} onClick={(e) => { e.preventDefault(); void deployContract() }}>Deploy ContribChain</button>
